...
class FiveSimBot {
    constructor() {
        ...
        console.log('🤖 5SIM Bot started with enhanced architecture!');
    }

    getCountryFlag(countryName) {
        const lowerName = countryName.toLowerCase().replace(/\s+/g, '');
        return this.countryFlags[lowerName] || '🌍';
    }
    ...
    async fiveSimRequest(endpoint, method = 'GET', params = {}) {
        try {
            ...
            const config = {
                ...
                url: url,
                headers: {
                    'Authorization': `Bearer ${this.config.FIVESIM_API_KEY}`,
            ...
            console.error(`5SIM API Error (${endpoint}):`, error.message);
            if (error.response) {
    ...
    async showCountries(chatId, messageId, page = 0) {
        try {
            const productsData = await this.fiveSimRequest('/guest/products/any/any');
            ...
            if (!productsData) {
                ...
                const errorText = `\`\`\`❌ *Gagal Mengambil Data Negara*\n\n` +
                    `📄 *Respon Error:*\n` +
                    `Gagal terhubung ke server 5SIM\`\`\``;
            ...
            Object.keys(productsData).forEach(countryCode => {
                const countryData = productsData[countryCode];
                if (countryData && typeof countryData === 'object') {
                    let hasAvailable = false;
                    Object.keys(countryData).forEach(operator => {
                        const operatorData = countryData[operator];
                        if (operatorData && typeof operatorData === 'object') {
                            Object.keys(operatorData).forEach(product => {
                                const productData = operatorData[product];
                    ...
                    if (hasAvailable && !countriesMap.has(countryCode)) {
            ...
            if (countries.length === 0) {
                ...
                await editPhotoCaption(this.bot, chatId, messageId, this.botLogo, '```❌ Tidak ada negara dengan stock tersedia saat ini```', errorKeyboard);
                return;
            }
            ...
            countriesOnPage.forEach(country => {
                ...
                keyboard.inline_keyboard.push([{
                    ...
                    callback_data: `country_${country.code}_page_0`
                }]);
            });
            ...
            const headerText = `\`\`\` 🌍 *Pilih Negara* (Hal ${page + 1}/${totalPages})\n\n` +
                `Total ${countries.length} negara tersedia.\n` +
    ...
    async showServices(chatId, messageId, data) {
        ...
        const countryCode = dataParts[0];
        const currentPage = parseInt(dataParts[1] || '0');
        
        try {
            const productsData = await this.fiveSimRequest(`/guest/products/${countryCode}/any`);
            ...
            if (!productsData) {
                ...
                const errorText = `❌ *Gagal Mengambil Data Layanan*\n\n` +
                    `📄 *Respon Error:*\n` +
                    `Gagal mengambil data layanan dari 5SIM`;
            ...
            Object.keys(productsData).forEach(operator => {
                const operatorData = productsData[operator];
                if (operatorData && typeof operatorData === 'object') {
                    Object.keys(operatorData).forEach(productCode => {
                        const productData = operatorData[productCode];
                        if (productData && productData.count > 0) {
                            const price = parseFloat(productData.cost || productData.price || 0);
                            const count = parseInt(productData.count || 0);
                            
                            if (!servicesMap.has(productCode)) {
                                servicesMap.set(productCode, {
                                    code: productCode,
                                    name: this.getProductName(productCode),
                                    price: price,
                                    count: count,
                                    operator: operator
                                });
                            } else {
                                const existing = servicesMap.get(productCode);
                                if (price < existing.price || (price === existing.price && count > existing.count)) {
                                    servicesMap.set(productCode, {
                                        code: productCode,
                                        name: this.getProductName(productCode),
                                        price: price,
            ...
            if (services.length === 0) {
                ...
                await editPhotoCaption(this.bot, chatId, messageId, this.botLogo, `\`\`\`❌ *Stock Habis di ${countryName}*\n\nSilakan pilih negara lain atau coba lagi nanti.\`\`\``, keyboard);
                return;
            }

            const priorityServices = services.sort((a, b) => {
            ...
            servicesOnPage.forEach(service => {
                const priceRub = service.price;
                const priceIdr = Math.ceil(priceRub * 150) + this.config.MARKUP_PROFIT;
                const name = service.name.toLowerCase();
                ...
                const text = `${emoji} ${service.name.toUpperCase()} - Rp ${priceIdr.toLocaleString('id-ID')} | Stok: ${service.count}`;
                keyboard.inline_keyboard.push([{
                    text,
                    callback_data: `service_${service.code}_${countryCode}_${priceIdr}_${service.name.replace(/\s/g, '_')}_${service.operator}`
                }]);
            });
            ...
            if (currentPage > 0) {
                navButtons.push({
                    text: '⬅️ Sebelumnya',
                    callback_data: `country_${countryCode}_page_${currentPage - 1}`
                });
            }
            ...
            const headerText = `${countryFlag} *Layanan di ${countryName}* (Hal ${currentPage + 1}/${totalPages})\n\n` +
    ...
    async confirmPurchase(chatId, messageId, data, userId) {
        ...
        const productCode = dataParts[1];
        const countryCode = dataParts[2];
        const price = parseInt(dataParts[3]);
        const serviceName = dataParts[4].replace(/_/g, ' ');
        const operator = dataParts[5] || 'any';
        ...
        const keyboard = {
            inline_keyboard: [
                [{ text: '✅ Beli Sekarang', callback_data: `buy_confirm_${productCode}_${countryCode}_${price}_${serviceName.replace(/\s/g, '_')}_${operator}` }],
                [{ text: '🔙 Kembali', callback_data: `country_${countryCode}_page_0` }]
            ]
        };
        ...
        const confirmText = `\`\`\`📱 *Konfirmasi Pembelian*\n\n` +
            `📧 Layanan: ${serviceName}\n` +
            `${countryFlag} Negara: ${countryName}\n` +
    ...
    try {
        ...
        if (hasActiveOrder) {
            ...
            try {
                await this.fiveSimRequest(`/user/cancel/${hasActiveOrder.orderId}`);
            } catch (e) {}
    ...
    async processPurchase(chatId, messageId, data, userId) {
        ...
        const productCode = dataParts[0];
        const countryCode = dataParts[1];
        const oldPrice = parseInt(dataParts[2]);
        const serviceName = dataParts[3].replace(/_/g, ' ');
        const operator = dataParts[4] || 'any';
        ...
        try {
            ...
            const productsData = await this.fiveSimRequest(`/guest/products/${countryCode}/${operator}`);
            
            if (!productsData || !productsData[operator] || !productsData[operator][productCode]) {
            ...
            const productData = productsData[operator][productCode];
            
            if (!productData || parseInt(productData.count || 0) === 0) {
                this.pendingOrders.delete(processingKey);
                const errorKeyboard = {
                    inline_keyboard: [
                        [{ text: '🔙 Pilih Layanan Lain', callback_data: `country_${countryCode}_page_0` }],
                        [{ text: '🏠 Menu Utama', callback_data: 'back_main' }]
                    ]
                };
                await editPhotoCaption(
            ...
            const priceRub = parseFloat(productData.cost || productData.price || 0);
            const finalPrice = Math.ceil(priceRub * 150) + this.config.MARKUP_PROFIT;
                ...
                const priceChangeKeyboard = {
                    inline_keyboard: [
                        [{ text: `✅ Lanjut Bayar Rp ${finalPrice.toLocaleString('id-ID')}`, 
                          callback_data: `buy_confirm_${productCode}_${countryCode}_${finalPrice}_${serviceName.replace(/\s/g, '_')}_${operator}` }],
                        [{ text: '🔙 Batal', callback_data: `country_${countryCode}_page_0` }]
                    ]
                };
            ...
            const orderResponse = await this.fiveSimRequest(`/user/buy/activation/${countryCode}/${operator}/${productCode}`);
            ...
            if (!orderResponse || !orderResponse.id) {
                ...
                if (errorInfo.type === "no_numbers") {
                    errorText += `📱 *Status:* Nomor Habis\n\n`;
                } else if (errorInfo.type === "no_balance") {
                    errorText += `💰 *Status:* Saldo Provider Habis\n\n`;
                } else if (errorInfo.type === "bad_service") {
                    errorText += `🔧 *Status:* Layanan Tidak Tersedia\n\n`;
                } else {
            ...
            const number = orderResponse.phone;

            const users = await this.db.loadUsers();
            const userIndex = users.findIndex(u => u.id === userId.toString());
            
            if (userIndex === -1) {
                await this.fiveSimRequest(`/user/cancel/${orderId}`);
                this.pendingOrders.delete(processingKey);
                await editPhotoCaption(
                    this.bot,
                    chatId,
    ...
    async startSMSMonitoring(userId, orderId) {
        ...
        const monitor = setInterval(async () => {
            ...
            try {
                ...
                const statusResponse = await this.fiveSimRequest(`/user/check/${orderId}`);

                if (statusResponse && statusResponse.status === 'RECEIVED' && statusResponse.sms && statusResponse.sms.length > 0) {
                    const smsCode = statusResponse.sms[0].code;
                    ...
                    if (currentOrders[userId] && currentOrders[userId].status === 'active') {
                        ...
                        await this.fiveSimRequest(`/user/finish/${orderId}`);
                        ...
                        const successText = `📨 *SMS Berhasil Diterima!*\n\n` +
                            ...
                            `${countryFlag} Negara: ${orderData.countryName}\n` +
                            `📅 Tanggal: ${timeInfo.date}\n` +
                            `🕐 Jam: ${timeInfo.time}\n\n` +
                            `✅ Transaksi selesai!\n` +
    ...
    async cancelOrder(chatId, messageId, data, userId) {
        ...
        try {
            ...
            while (!cancelSuccess && attempts < maxRetries) {
                ...
                try {
                    const cancelResponse = await this.fiveSimRequest(`/user/cancel/${orderId}`);
                    
                    if (cancelResponse && cancelResponse.status === 'CANCELED') {
                        cancelSuccess = true;
                        console.log(`✅ Cancel berhasil untuk order ${orderId}`);
                    } else {
                        console.log(`❌ Cancel attempt ${attempts} gagal:`, cancelResponse);
    ...
    async showOrderHistory(chatId, messageId, userId) {
        ...
        displayHistory.forEach((order, index) => {
            ...
            historyText += '   ' + countryFlag + ' ' + (order.countryName || order.country || 'Unknown') + '\n';
    ...
    async showActiveOrders(chatId, messageId, userId) {
        ...
        const activeText = `\`\`\`📋 Pesanan Aktif\n\n` +
            ...
            `${countryFlag} Negara: ${order.countryName || order.country}\n` +
...
Pilih menu di bawah:\`\`\``;
    ...
    async sendTestimoniToChannel(orderData, smsCode) {
        try {
            ...
            const testimoniText = `🎉 *TRANSAKSI BERHASIL* 🎉\n\n` +
                `👤 Customer: ${userName}\n` +
                `📧 Layanan: ${orderData.serviceName}\n` +
                `${countryFlag} Negara: ${orderData.countryName || orderData.country}\n` +
                `📱 Nomor: +${hiddenNumber}\n` +
                `🔑 Kode: ${hiddenOTP}\n` +
                `💰 Harga: Rp ${orderData.price.toLocaleString('id-ID')}\n` +
    ...
    getOrderErrorMessage(orderResponse) {
        ...
        if (lowerMsg.includes('no free phones') || lowerMsg.includes('no numbers')) {
            return {
                message: errorMsg || "No phone numbers available",
                type: "no_numbers",
                solution: "Nomor habis. Coba negara lain atau tunggu restock."
            };
        }
        
        if (lowerMsg.includes('not enough balance') || lowerMsg.includes('no balance')) {
            return {
                message: errorMsg || "Provider balance insufficient",
                type: "no_balance",
                solution: "Saldo provider habis. Hubungi admin."
            };
        }
        
        if (lowerMsg.includes('bad service') || lowerMsg.includes('service unavailable')) {
            return {
                message: errorMsg || "Service not available",
                type: "bad_service",
                solution: "Layanan tidak tersedia. Coba lagi nanti."
            };
        }
        
        return {
            message: errorMsg || "Unknown error from provider",
            type: "generic",
            solution: "Coba lagi dalam beberapa menit atau hubungi admin."
        };
    }
    ...
    getProductName(productCode) {
        ...
        return productNames[lowerCode] || productCode.charAt(0).toUpperCase() + productCode.slice(1);
    }
}
...
console.log('🚀 5SIM Bot dimulai dengan arsitektur tingkat perusahaan');